...
public class GptService {
    ...
    public String createTaxiPrompt(double realDistance, int realFee, int realUsetime, double predictDistance, int predictFee, int predictUsetime ) {
        String prompt = String.format(
                "택시비 바가지는 택시요금을 원래의 가격보다 부당하게 높게 받는것을 택시요금 바가지라고 해.[output.]\n" +
                        "예상 요금: %d 원, 예상 거리는 %.2f km, 예상 시간은 %d 분이였어. 실제 요금: %d 원, 거리는 %.2f km, 예상 시간은 %d 분이 걸렸어. [output.]\n" +
                        "택시비 바가지 우선순위 기준은" +
                        "1.실제요금이 예상 요금보다 30%% 이상 높은지\n " +
                        "2.실제거리가 예상 거리보다 30%% 이상 많은지\n" +
                        "3.실제시간이 예상 시간보다 30%% 이상 많은지\n" + "를 기준으로 둘게[output.]\n" +
                        "이 데이터에 대해 분석해주고, 그리고 실제와 예상이 얼마나 차이나는지 알려줘." +
                        "(필수)또 너가 판단 했을 떄 택시비 바가지의 위험정도를 적정,의심,위험,매우위험 정도로 판단해줘(내가 라는 말 사용 금지, 제가로 대체)"
    ...
    public String createProductPrompt(ProductInnerDto productInnerDto){
        String prompt = String.format(
                "농수산물 바가지는 가격을 원래의 가격보다 부당하게 높게 받는것을 농수산물 바가지라고 해.[output.]\n" +
                        ...
                        "이 데이터에 대해 분석해주고, 그리고 실제와 평균 가격들이 얼마나 차이나는지 알려줘." +
                        "(필수)또 너가 판단 했을 떄 농수산물 바가지의 위험정도를 저렴,적정,의심,위험,매우위험 정도로 판단해줘",
    ...
    public String createFoodPrompt(FoodResponseDto foodResponseDto, int realPrice){
        String prompt = String.format(
                        "음식의 전국 평균 가격 = %d원, 지역 평균 가격 = %d원, 지역 최소 가격 = %d원, 지역 최대 가격 = %d원, 실제 가격 = %d원." +
                                "음식 바가지는 음식의 가격을 원래의 가격보다 부당하게 높게 받는 것을 음식 바가지라해.[output.]\n" +
                        "너가 위의 데이터를 종합해서 음식의 가격이 바가지인지 아닌지 판단해줘.\n" +
                        "가격이 0원인게 있다면 그 값은 없는 값이야. 배제하고 판단해줘.[output.]\n" +
                        "답변 형식은 최대한 정중한 말투로 중립을 지키면서 기준에 의해 판단해서 답변을 해줘, 그리고 실제 가격과 (지역,전국)평균 가격들이 얼마나 비싼지 알려줘." +
                        "또 너가 판단 했을 떄 음식 바가지의 위험정도를 저렴,적정,의심,위험,매우위험 정도로 판단해줘.(\"\"안에 위험정도를 나타내주고, 내가 라는 말 사용 금지, 제가로 대체)"
